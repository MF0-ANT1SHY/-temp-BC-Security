...
from squirRL_environment import eth_env
from unifiedenv import setup_logging
...
STEPS_CONFIG = [
    {"name": "long", "steps": 10000}
]

logger = setup_logging("simulate", "info")
...
def simulate_random_actions(params):
    ...
    for step in range(steps):
        ...
        state, reward, _, _ = env.step(state, action)
        accumulated_reward += reward
        
        if (step + 1) % RECORD_INTERVAL == 0:
            attacker_fraction = env._attacker_block / (env._attacker_block + env._honest_block)
...
#     for step in range(steps):
#         for action in (2, 3, 1):
#             state, reward, _, _, _ = env.step(state, action, 0, move=True)
#             accumulated_reward += reward
...
def run_parallel_simulation(steps_config, fuzzing=False):
    ...
    params_list = [(alpha, steps) for alpha in ALPHA_RANGE for _ in range(TRIALS_PER_ALPHA)]
    ...
    with Pool(processes=num_processes) as pool:
        ...
        simulation_func = simulate_random_actions
...
def plot_results(results, steps_config):
    ...
    plt.tight_layout()
    plt.savefig(f'.plot/alpha_convergence_{steps_config["name"]}.png', 
                dpi=300, 
                bbox_inches='tight')
    plt.close()
    ...
    plt.legend(loc='upper left')
    plt.tight_layout()
    plt.savefig(f'.plot/rewards_{steps_config["name"]}.png', 
                dpi=300, 
                bbox_inches='tight')
    plt.close()

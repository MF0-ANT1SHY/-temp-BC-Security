...
parser = argparse.ArgumentParser(description='Bitcoin Mining Strategy Simulation')
parser.add_argument('--episodes', type=int, default=100, help='Number of episodes to train')
parser.add_argument('--load-model', action='store_true', help='Whether to load a saved model')
parser.add_argument('--load-best', action='store_true', help='Load saved best model')
parser.add_argument('--alpha', type=float, default=0.4, help='Hash power fraction of attacker')
parser.add_argument('--gamma', type=float, default=0.5, help="Follower's fraction")
parser.add_argument('--hidden-block', type=int, default=20, help='Maximum hidden block of attacker')
parser.add_argument('--rule', choices=['longest', 'GHOST'], default='longest', help='Bitcoin rule to use')
parser.add_argument('--stale-rate', type=float, default=0.0, help='Stale block rate')
parser.add_argument('--dev', type=float, default=0.0, help="Alpha's fluctuation rate")
parser.add_argument('--know-alpha', action='store_true', default=True, help='If agent knows current alpha')
parser.add_argument('--random-process', choices=['iid', 'brown'], default='iid', help='Random process type')

args = parser.parse_args()

num_episodes = args.episodes
load_model = args.load_model
load_best_model = args.load_best
ALPHA = args.alpha
GAMMA = args.gamma
HIDDEN_BLOCK = args.hidden_block
rule = args.rule
stale_rate = args.stale_rate
DEV = args.dev
know_alpha = args.know_alpha
random_process = args.random_process
